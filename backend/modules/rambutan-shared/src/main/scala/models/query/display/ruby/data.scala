...
case class ArrayDisplay(id: String, elements: List[DisplayStructContainer]) extends DisplayStruct with NoBodyStruct {
  ...
  def chunks(indents: Int, context: List[String]) = {
    ...
    val (_, calculatedElements) = CallArg.calculateIndexes(id, elements, RubyHighlightType.ArrayElement)
    ...
    List(
      ...
      DisplayItem.EmptyBody(id, RubyHighlightType.ArrayElement, context) :: Nil,
      withFlag(multiline) {
  ...
  def applyChildren(children: Map[BuilderEdgeType, List[DisplayStructContainer]]): DisplayStruct = {
    this.copy(
      elements = this.elements ++ children.getOrElse(RubyBuilderEdgeType.ArrayElement, Nil))
  }
...
case class HashPairDisplay(id: String, name: Option[String], inner: Option[DisplayStruct]) extends DisplayStruct with NoBodyStruct {
  ...
  def chunks(indents: Int, context: List[String]) = {
    ...
    val innerChunks = inner match {
      ...
      case None    => DisplayItem.EmptyItem(id, RubyHighlightType.PairValue, context) :: Nil
    }
  ...
  def applyChildren(children: Map[BuilderEdgeType, List[DisplayStructContainer]]): DisplayStruct = {
    val inner = children.getOrElse(RubyBuilderEdgeType.PairValue, Nil).headOption.map(_.struct)
...
case class HashDisplay(id: String, properties: List[DisplayStruct]) extends DisplayStruct with NoBodyStruct {
  ...
  def chunks(indents: Int, context: List[String]) = {
    ...
    List(
      ...
      propertiesChunks,
      StaticDisplayItem.NewLine(1, indents + DisplayChunk.TabSize) :: Nil,
      DisplayItem.EmptyBody(id, RubyHighlightType.HashElement, context) :: Nil,
      StaticDisplayItem.NewLine(1, indents) :: Nil,
  ...
  def applyChildren(children: Map[BuilderEdgeType, List[DisplayStructContainer]]): DisplayStruct = {
    val props = children.getOrElse(RubyBuilderEdgeType.HashElement, Nil).map(_.struct)
