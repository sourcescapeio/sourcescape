...
class LocalGitService @Inject() (
  ...
  repoDataService:      LocalRepoDataService)(implicit ec: ExecutionContext, mat: akka.stream.Materializer) {

  def getGitRepo(repoPath: String): Future[GitServiceRepo] = {
    getGitRepoInner(repoPath)
  }

  private def getGitRepoInner(repoDir: String): Future[LocalRepoContainer] = {
    Future {
      val builder = new FileRepositoryBuilder()
      val repo = builder.setGitDir(new File(repoDir + "/.git")).setMustExist(true).build()
